...
var s = '5000000';
var totalmoney = 5000000;
var money_2;

var buy = function (price,count) {
    totalmoney = totalmoney - price*count;
    money_2 = totalmoney - 0.4*300*price*count;
};
...
$(document).ready(function () {
    $.get("http://121.201.68.143/news/get_part?n=1&k=3", function (data) {
        for(var i = 0;i<3;i++) {
            var itemHTML = '<a href="article.html?id='+(i+1)+'"><li><div class="item-content"><div class="item-media"><img src="img/daily-newspaper-.png" width="44"></div> <div class="item-inner"> <div class="item-title-row"> <div class="item-title">' + data[i].title + '</div> </div> <div class="item-subtitle">Beatles</div> </div> </div> </li></a>'

            $('#index-news-ul').append(itemHTML);
        };
    });


    $('#signin').on('click', function () {
        $.post("http://121.201.68.143/customer/register/",{uname:$('#uname').val(), pwd:$('#pwd').val()},
        function (data) {
            console.log(data);
        });
    });
    ...
    $('#login').on('click', function () {
        $.post("http://121.201.68.143/customer/login/",{uname:$('#uname').val(), pwd:$('#pwd').val()},
            function (data) {
                if(data.state = 200){
                    token = data.token;
                    username = $('#uname').val();
...
myApp.onPageInit('detail', function (page) {
    ...
    $.get("3.json", function (data) {
        ...
        for(var i=0; i < 1000; i++){
            newData[i] = [i,data.yesterday[i]];
            newData_2[i] = [i,data.avg[i]];
        }
        console.log(newData);
    ...
    var open = ll;
    var is_notice = 0;
    var notice = function (price_last,price,up,low) {
        if((price < low)&&(price<=price_last)&&(open>0.5*(price+low))){
            open = 0.5*(price+low);
        }
        if(price_last<=open && price>=open){
    ...
    var notice_2 = function (price_last,price,mid,low_solid) {
        if(is_notice == 1 ){
            if(price >= mid){
                myApp.addNotification({
                    title: '提醒',
            ...
            if(price_last >= low_solid && price <= low_solid){
                myApp.addNotification({
                    title: '提醒',
                    message: '平仓'
                });
    ...
    var add = function () {
        setInterval(function () {
            ...
            notice(data.avg[m-1],data.avg[m],hh,ll);
            notice_2(data.avg[m-1],data.avg[m],mid,l);
        },1);
    };
    add();
...
myApp.onPageInit('form', function (page) {
    ...
    $$('#signin-btn').on('click', function () {
        $.post("http://121.201.68.143/customer/register/",
               {uname:145, pwd:78979 },
               function (data) {
                 console.log(data);
...
myApp.onPageInit('news', function (page) {
    ...
    $.get("http://121.201.68.143/news/get_part?n=1&k=5", function (data) {
        for(var i = 0;i<5;i++) {
            var itemHTML = '<a href="article.html?id='+(i+1)+'"><li><div class="item-content"><div class="item-media"><img src="img/daily-newspaper-.png" width="44"></div> <div class="item-inner"> <div class="item-title-row"> <div class="item-title">' + data[i].title + '</div> </div> <div class="item-subtitle">Beatles</div> </div> </div> </li></a>'
    ...
    ptrContent.on('refresh', function (e) {
        setTimeout(function () {
            $.get("http://121.201.68.143/news/get_part?n="+n+"&k=5", function (data) {
                for(var i = 0;i<5;i++) {
                    var itemHTML = '<a href="article.html?id='+(i+n)+'"><li><div class="item-content"><div class="item-media"><img src="img/daily-newspaper-.png" width="44"></div> <div class="item-inner"> <div class="item-title-row"> <div class="item-title">' + data[i].title + '</div> </div> <div class="item-subtitle">Beatles</div> </div> </div> </li></a>'

                    $('#news-ul').prepend(itemHTML);
                };
                n = n+5;
            });
...
myApp.onPageInit('price', function (page) {
    $.get("url", function (data) {
        var itemHTML =  '<li class="item-content">'+
            '<div class="item-inner">'+
            '<div class="item-title">IH1609</div>'+
            '<div class="item-after"><span>2175.0</span><span style="color: #2d882d">-11.2</span><span style="color: #2d882d">-0.51%</span></div>'+
            '</div>'+
            '</li>'
    })
});
...
myApp.onPageInit('setting', function (page) {
    $('#setting-next1').click(function (e) {
        $('#tab2').addClass("active");
        $('#tab-btn-2').addClass("active");
        $('#tab1').removeClass("active");
        $('#tab-btn-1').removeClass("active");
    });
    $('#setting-next2').click(function (e) {
        $('#tab3').addClass("active");
        $('#tab-btn-3').addClass("active");
        $('#tab2').removeClass("active");
        $('#tab-btn-2').removeClass("active");
    });
    $('#setting-next3').click(function (e) {
        $('#tab4').addClass("active");
        $('#tab-btn-4').addClass("active");
        $('#tab3').removeClass("active");
        $('#tab-btn-3').removeClass("active");
    });
    $('#setting-last2').click(function (e) {
        $('#tab1').addClass("active");
        $('#tab-btn-1').addClass("active");
        $('#tab2').removeClass("active");
        $('#tab-btn-2').removeClass("active");
    });
    $('#setting-last3').click(function (e) {
        $('#tab2').addClass("active");
        $('#tab-btn-2').addClass("active");
        $('#tab3').removeClass("active");
        $('#tab-btn-3').removeClass("active");
    });
    $('#setting-last4').click(function (e) {
        $('#tab3').addClass("active");
        $('#tab-btn-3').addClass("active");
        $('#tab4').removeClass("active");
        $('#tab-btn-4').removeClass("active");
    });
});

myApp.onPageInit('analysis', function (page) {
        ...
        series: [
            {
                name: '1',
                type:'line',
                data:[['2016-06-23','0'],['2016-06-24','0.8']]
            },{
                name: '2',
                type:'line',
                data:[['2016-06-23','0'],['2016-06-24','-1.14']]
            }
        ]
    };
    myChart.setOption(option);
...
myApp.onPageInit('account', function (page) {
    // setInterval(function () {
    //     num = Math.random()*200+2900;
    //     $(".yk").empty();
    //     $(".yk").append('￥'+num.toFixed(1));
    //     $("#total_money").empty();
    //     $("#total_money").append(totalmoney+num.toFixed(1));
    // },1000);

    // $("#money_2").append(money_2);
    // console.log('..............');
    // var m = money_2/totalmoney
    // console.log(m.toFixed(2));
    // $("#rate").append(m.toFixed(2)*100+'%');
    // $.post("http://121.201.68.143/account/create_virtual_account/", {username:username, token:token},
    // function (data) {
    //     console.log(data);
    // });
    // $.post("http://121.201.68.143/account/create_virtual_account/")
});

myApp.onPageInit('article', function (page) {
    id = page.query.id;
    $.get("http://121.201.68.143/news/get_part?n="+id+"&k=1", function (data) {
        $('h1').append(data[0].title);
        $('p').append(data[0].content);
    })
});

myApp.onPageInit('bbs', function (page) {
    $.get("http://121.201.68.143/bbs/showlist", function (data) {
        console.log(data);
        for(var i = 0;i < data.length;i++){
            var item = '<li>'+
            '<a href="bbsarticle.html?id='+data[i].id+'" class="item-link item-content">'+
                '<div class="item-inner">'+
                '<div class="item-title-row">'+
                '<div class="item-title">'+data[i].title+'</div>'+
                '<div class="item-after">'+data[i].author+'</div>'+
                '</div><div class="item-subtitle">'+data[i].category+'</div><div class="item-text">'+data[i].date+'</div></div></a></li>';
            $('ul').append(item);
        }
    })
});

myApp.onPageInit('bbsarticle', function (page) {
    var id = page.query.id;
    $.get("http://121.201.68.143/bbs/showarticle/?id="+id, function (data) {
        console.log(data);
        $('h1').append(data.article.title)
        $('p').append(data.article.content)
    })
});

myApp.onPageInit('school_list', function (page) {
    var category = page.query.id;
    $.get("http://121.201.68.143/school/get_category/?category=%E5%9F%BA%E7%A1%80%E7%9F%A5%E8%AF%86", function (data) {
        console.log(data);
        for(var i = 0; i < data.length; i++) {
            var item = '<a href="school_article.html?id='+data[i].id+'"><li><div class="item-content"><div class="item-inner"><div class="item-title-row"><div class="item-title">' + data[i].title + '</div></div></div></div> </li></a>';
            $('ul').append(item);
        };
    });
});

myApp.onPageInit('school_article', function (page) {
    var id = page.query.id;
    console.log(id);
    $.get("http://121.201.68.143/help/?id="+id, function (data) {
        console.log(data);
        $('h1').append(data[0].title);
        $('p').append(data[0].content);
    });
...
myApp.onPageInit('deal', function (page) {
    ...
    $('#buy-btn').click(function () {
        buy($('#buy-price-in').val(),$('#buy-count-in').val());
         console.log($('#buy-price-in').val());
         console.log($('#buy-count-in').val());
         console.log($('#buy-price-in').val()*$('#buy-count-in').val())
        //var m = $('#buy-price-in').val()*$('#buy-count-in').val();
        //total_money = total_money - m;
        console.log(a+1);
        console.log(typeof(totalmoney+1));
